...
public class MatriculaDAO {
    ...
    public void insertar(Matricula matricula) throws SQLException {
        ...
        try {
            ps = cnn.prepareStatement("INSERT INTO matricula "
                    ...
                    + ",fecha)"
                    + "VALUES (?,?,?,?)");
            ps.setString(1, matricula.getCodigo());
            ps.setString(2, matricula.getAlumno().getCodigo());
            ps.setString(3, matricula.getProgramacion().getCodigo());
            ...
            for (int i = 0; i < datos.length; i++) {
                ps1 = cnn.prepareStatement(
                        ...
                        + ",codigo_curso)"
                        + "VALUES (?,?)");
                ps1.setString(1, matricula.getCodigo());
    ...
    public Matricula buscarMatricula(String idMatricula) throws SQLException {
        ...
        try {
            ps1 = cnn.prepareStatement("SELECT * FROM matricula WHERE codigo = ?");
            ps1.setString(1, idMatricula);
            rs = ps1.executeQuery();

            if (rs.next()) {
                String idAlumno = rs.getString("codigo_alumno");
                Alumno alumno = AlumnoDAO.getInstancia().buscarAlumno(idAlumno);
                String idProgramacion = rs.getString("codigo_programacion");
                ...
                ps2 = cnn.prepareStatement("SELECT * FROM detalleMatricula where codigo = ?");
                ps2.setString(1, idMatricula);
                rs = ps2.executeQuery();
                while (rs.next()) {
                    String idCurso = rs.getString("codigo_curso");
    ...
    public ArrayList<Matricula> mostrarMatriculas() throws SQLException {
        ...
        try {
            ...
            rs1 = ps.executeQuery();
            while (rs1.next()) {
                String idMatricula = rs1.getString("codigo");
    ...
    public void eliminar(String idMatricula) throws SQLException {
        ...
        PreparedStatement ps = null;
        try {
            ps = cnn.prepareStatement("DELETE FROM matricula WHERE codigo = ?");
